...
-- Create a more restrictive policy that prevents direct table access for public users
-- Public users can only access data through the security definer function
CREATE POLICY "Authenticated users can view their own funnels" 
ON interactive_funnels 
FOR SELECT 
...
ALTER VIEW public.shared_funnel_steps_safe ENABLE ROW LEVEL SECURITY;

-- Create policy for the view
CREATE POLICY "Public can view shared funnel steps safely" 
ON public.shared_funnel_steps_safe 
FOR SELECT 
USING (true);
