...
-- and replacing with a more secure function approach

-- Drop the problematic view that has RLS enabled
DROP VIEW IF EXISTS public.shared_funnel_steps_safe;

-- Drop the existing function so we can recreate it with new return type
DROP FUNCTION IF EXISTS public.get_shared_funnel_safe(text);

-- Create the updated secure function to include steps data in a single call
